!program for calculating backbone trace of nucleosomal DNA

!this code first alligns the trajectory wrt some reference structure 
!using quaterion method

!The code is written by Prabir Khatua
!date: 21st November, 2014
!Modified on: 5th October, 2021

!Usage: f95 BASEPAIR_SUPERIMPOS_WITH_TIME.F90 or gfortran BASEPAIR_SUPERIMPOS_WITH_TIME.F90
...
!The coe will generate output files for the backbone trace at different 
!time. This generates such a trace separately for right and left halves 
!of the nucleosomal DNA

!The code is written to analyze the dcd trajectories
!------------------------------------------------------------------------------
program rmsd
...
character(len=200):: pdbfile,Prefix,Seq
character(len=200),dimension(:),allocatable:: tail1,tail2
...
!------------------------------------------------------------------------------
write(*,'(1x,a)')'Type the prefix for output files'
read(*,'(a)')Prefix
write(*,*)Prefix
write(*,'(1x,a)')'Enter the total time length in microsecond'
read(*,*)Time
write(*,*)'Time===>',Time
write(*,'(1x,a)')'Enter the no of output files'
read(*,*)nout
write(*,*)'nout====>',nout

Time = Time/real(nout)

allocate(tail1(nout),tail2(nout))

do i = 1,nout
write(Seq,'(f4.1)')i*Time
tail1(i) = trim(prefix)//trim(adjustl(Seq))//"ms_"//"pav_tail1_proj.dat"
tail2(i) = trim(prefix)//trim(adjustl(Seq))//"ms_"//"pav_tail2_proj.dat"
write(*,*)tail1(i),tail2(i)
enddo
!------------------------------------------------------------------------------
...
read(*,*)nskip
write(*,*)'nskip======>',nskip
write(*,'(1x,a)')'Enter the first and last atom IDs u want to analyse'
read(*,*)ifirst,ilast
...
write(*,*)'nread====>',nread
nused=nused+1
open(t1unit,file=tail1(nused),status='unknown',form='formatted',iostat=ios)
open(t2unit,file=tail2(nused),status='unknown',form='formatted',iostat=ios)
!------------------------------------------------------------------------------
