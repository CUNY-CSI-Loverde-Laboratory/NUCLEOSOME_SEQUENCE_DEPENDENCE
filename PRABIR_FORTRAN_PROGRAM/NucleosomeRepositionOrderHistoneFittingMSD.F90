!Program for calculating mean square displacement of translocation position of Nucleosome
...
!Modified on: 11th December, 2021
!RMS Fitting version modified on: 9th May, 2022

!Usage: f95 NucleosomeRepositionOrderHistoneFittingMSD.F90 or gfortran NucleosomeRepositionOrderHistoneFittingMSD.F90
...
real(kind=8),dimension(:,:),allocatable:: TransLocPosition
real(kind=8),dimension(:),allocatable:: MSD,Norm
...
allocate(PxBaseRef(NumDnaBasePair))
...
write(*,*)'# of selected histone heavy atom====>',NpairHistone
write(*,*)'# of selected H31 heavy atom=======>',NpairH31
write(*,*)'# of selected H32 heavy atom=======>',NpairH32
write(*,*)
...
ntotframes=ntotframes+nframes
close(inunit)
enddo

write(*,*)'ntotframes=====>',ntotframes

!memory allocation
allocate(MSD(0:ntotframes-1),Norm(0:ntotframes-1))
allocate(TransLocPosition(NumDnaBasePair,ntotframes))
!------------------------------------------------------------------------------
nread=0
nused=0
...
if(SignDet <= 0.0)then
        TransLocPosition(ii,nused) = acos(Angle)/Lambda
else
        TransLocPosition(ii,nused) = -acos(Angle)/Lambda
endif
enddo
...
DotX = TransLocPosition(i,k) - TransLocPosition(i,j)
MSD(m) = MSD(m) + DotX*DotX
Norm(m) = Norm(m) + 1.d0
enddo
enddo
enddo
...
write(ounit,'(2f12.6)')i*dt/1000000.0,MSD(i)
enddo
