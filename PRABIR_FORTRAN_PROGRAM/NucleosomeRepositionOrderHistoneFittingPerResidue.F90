!Program for calculating average translatiion and rotational  order parameter for 
!Nucleosome sliding
...
!Usage: f95 NucleosomeRepositionOrderHistoneFittingPerResidue.F90 
!       or gfortran NucleosomeRepositionOrderHistoneFittingPerResidue.F90
...
real(kind=8),dimension(:),allocatable:: SrotX,SrotY,Strans
...
integer,dimension(:),allocatable:: H31_AtomId,H32_AtomID,HistoneAtomId
...
real(kind=8):: CrosX,CrosY,CrosZ,Srot
...
allocate(SrotX(NumDnaBasePair),SrotY(NumDnaBasePair),Strans(NumDnaBasePair))
!------------------------------------------------------------------------------
...
SrotX(i) = SrotX(i) + cos(Angle)
SrotY(i) = SrotY(i) + sin(Angle)
...
!------------------------------------------------------------------------------
do i=1,NumDnaBasePair
Strans(i) = Strans(i)/(dble(nused)*Lambda)
SrotX(i) = SrotX(i)/dble(nused)
SrotY(i) = SrotY(i)/dble(nused)

Srot = datan(SrotY(i)/SrotX(i))/pi
write(ounit,'(i6,1x,2f8.3)')i,Strans(i),Srot
enddo
